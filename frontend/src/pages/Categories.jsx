...
const Categories = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (!editing) {
        const created = await createCategory({ storeId, name: form.name, code: form.code });
        setRows((prev) => [created, ...prev]);
      } else {
        const updated = await updateCategory(editing._id, { name: form.name, code: form.code });
  ...
  return (
    <Box>
      ...
      <Paper elevation={0} sx={{ mt: 2, p: 2 }}>
        <Filters>
          <Search value={query} onChange={setQuery} placeholder="Search categories" />
          <TextField
            select
            size="small"
            label="Store"
              ...
              {filtered.map((row) => (
                <TableRow key={row._id}>
                  ...
                  <TableCell align="right">
                    ...
                    <IconButton size="small" onClick={() => handleDeactivate(row._id)}>
