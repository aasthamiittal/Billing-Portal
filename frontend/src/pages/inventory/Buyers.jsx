...
import { createBuyer, deleteBuyer, fetchBuyers, updateBuyer } from "../../services/inventoryService";
...
const Buyers = () => {
  ...
  useEffect(() => {
    const load = async () => {
      if (!storeId) return;
      const data = await fetchBuyers(storeId);
      setRows(data || []);
    };
    load();
  }, [storeId]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (!editing) {
        const created = await createBuyer({ storeId, ...form });
        setRows((prev) => [created, ...prev]);
      } else {
        const updated = await updateBuyer(editing._id, form);
  ...
  return (
    <Box>
      ...
      <Paper elevation={0} sx={{ mt: 2, p: 2 }}>
        <Filters>
          <Search value={query} onChange={setQuery} placeholder="Search buyers" />
          <TextField
            select
            size="small"
            label="Store"
      ...
      {!filtered.length ? (
        <EmptyState title="No buyers" description="Create your first buyer." />
      ) : (
        <TableWrapper>
          <Table>
            <TableHead>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editing ? "Edit Buyer" : "Create Buyer"}</DialogTitle>
        <DialogContent>
